...
`LogCspViolation` logs CSP violations as `warning`. The message names the violated directive, the blocked URL and the page, and the log context holds the full raw report under `report`. `LogReport` logs every other report type as `info`, with the full raw report in the log context. Neither is registered automatically.
